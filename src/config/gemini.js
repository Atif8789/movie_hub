...
const getAPIKey = () => {
  const apiKey = 'AIzaSyCAyWLMcRBhE85rPxTX7De4Pb-RiD4vo2I'
...
export const getMoodBasedRecommendations = async (mood,watchlist) => {
    console.log("hello2 ",watchlist)
    let str = watchlist.map((movie)=>movie.title).join(', ')
    console.log("str ",str)
  try {
    const model = await getGeminiModel();
    ...
    Please recommend 8 movies that match this ${mood.name} perfectly from watchlist - ${str} and also add from your side. Focus on popular, well-known movies that are easily available. For each movie, provide:
    - Title
    - Year (if known)
